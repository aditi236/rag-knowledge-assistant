...
variables, so behaviour changes without code changes (the "12-factor app" rule: config in the environment).""",
    [
        ...
        (11, 12, r"""`@dataclass(frozen=True)` makes `Settings` an immutable record: once created, nobody can change a value by accident while the app runs. Tests create their own `Settings(...)` with overrides instead of touching globals."""),
        (13, 13, r"""**Where the index is saved.** `os.getenv("INDEX_DIR", "storage/index")` means "use the env var if set, otherwise this default"."""),
        (14, 14, r"""**Embedding model.** `bge-small-en-v1.5` produces 384-number vectors, runs on a plain CPU, and downloads as a small ONNX file. It is a strong quality-for-size choice. Changing it means re-indexing everything (vectors from different models are not comparable) and re-calibrating `MIN_SCORE`."""),
        (15, 15, r"""**Which answer generator to use:** `auto` (Claude if an API key exists, else the offline extractive fallback), or force `claude` / `extractive`."""),
        (16, 16, r"""**Which Claude model.** Defaults to `claude-opus-5`. Set `LLM_MODEL=claude-sonnet-5` for a cheaper option; no code change needed."""),
        (17, 18, r"""**Chunking knobs.** `chunk_size` is in characters (800 is roughly 150 to 200 words, about 200 tokens). `chunk_overlap` (120, about 15%) repeats the end of one chunk at the start of the next, so a fact that straddles a boundary still appears whole in at least one chunk."""),
        (19, 19, r"""**`top_k`: how many chunks are retrieved** and sent to the model. Too small: you miss the answer. Too large: you pay for noise and distract the model."""),
        (20, 20, r"""**`min_score`: the "I don't know" threshold.** Retrieval always returns the *closest* chunks even for nonsense questions, so we need a cut-off. It was calibrated on the sample data: off-topic questions scored up to 0.52, genuine matches scored 0.64 or more, so 0.58 sits in the gap. This number depends on the embedding model and the corpus; re-measure with `python -m eval.run_eval` if either changes."""),
        (21, 21, r"""Maximum tokens Claude may produce. This budget also covers the model's internal reasoning on models that think by default, so it is set generously (4096) to avoid truncated answers."""),
    ],
)
...
This is what lets us search by meaning instead of by keyword.""",
    [
        ...
        (19, 22, r"""Constructor. The import sits **inside** the method (a *lazy import*), so importing this module, and running unit tests, does not load the heavy library or trigger a model download. `TextEmbedding(...)` downloads the model the first time (cached afterwards) and loads it into memory."""),
...
Also contains an offline fallback so the whole project runs without an API key.""",
    [
        ...
        (20, 21, r"""The `LLM` interface: any object with `generate(question, results) -> str`. The real model, the offline fallback and the test fake all satisfy it."""),
        (24, 24, r"""`build_user_message` formats the retrieved chunks and the question into the text sent as the user turn."""),
        (25, 28, r"""A generator expression numbers each passage starting at 1 (`enumerate(..., start=1)`, because humans and models say "passage 1", not "passage 0") and prints `[n] (source: file)` followed by the chunk text. `"\n\n".join(...)` puts a blank line between passages."""),
        (29, 29, r"""Wrap the passages in `<context>` tags, then put the question **after** the context. Clear delimiters help the model separate reference material from the question (and are a first step against prompt injection hidden in documents; they reduce, not eliminate, that risk). Putting the question last is recommended practice for long contexts."""),
        (32, 36, r"""`ClaudeLLM` remembers the model name and token limit. Note `_client` starts as `None`: the client is created **lazily**, so the app can start, pass health checks and accept uploads even with no API key. A missing key only surfaces when someone asks a question."""),
        (38, 40, r"""On the first call create the client. `anthropic.Anthropic()` with no arguments reads credentials from the environment (`ANTHROPIC_API_KEY`); the key is never written in code."""),
        ...
        (48, 49, r"""If the model's safety systems declined (`stop_reason == "refusal"`), the reply may be empty, so return a safe message instead of crashing. Always check `stop_reason` before trusting `content`."""),
        (50, 50, r"""`response.content` is a *list of blocks* (thinking blocks, text blocks, ...). We keep only blocks whose `type` is `"text"` and join their `.text`. Checking the type first avoids attribute errors on non-text blocks."""),
        (53, 57, r"""`ExtractiveLLM` is a zero-cost offline fallback: no model, it simply returns the best-matching chunk verbatim plus a `[1]` citation. It proves retrieval works on its own, lets the demo run with no key, and makes CI cheap."""),
        (60, 61, r"""`build_llm` is a small **factory function**: it decides which implementation to use, based on config."""),
        (62, 63, r"""In `auto` mode, use Claude if `ANTHROPIC_API_KEY` is set, otherwise the extractive fallback. (If you authenticate another way, for example `ant auth login`, set `LLM_PROVIDER=claude` explicitly.)"""),
        (64, 67, r"""Explicit choices: return the matching implementation."""),
        (68, 68, r"""Any other value is a configuration error: fail immediately and loudly at start-up, not silently later."""),
    ],
)
...
in two flows: **ingest** (index documents) and **ask** (retrieve, then generate).""",
    [
        ...
        (29, 33, r"""**Dependency injection**: the constructor *receives* its embedder, store, LLM and settings instead of creating them. In production `build_pipeline` (bottom of file) passes real ones; in tests we pass fakes. This is why 33 tests run in half a second with no network."""),
        ...
        (62, 65, r"""`build_pipeline` is the **composition root**: the one place where concrete classes are chosen and connected. It builds the real embedder, loads any previously saved index from disk (so restarts do not lose data), creates the LLM from settings, and assembles the pipeline."""),
    ],
)
...
APP["app/api.py"] = (
    ...
    [
        ...
        (11, 13, r"""Project imports: settings, loader helpers, and the pipeline plus its builder."""),
        (15, 15, r"""Upload size cap: 5 MB. Without a limit, one huge upload could exhaust memory."""),
        (17, 17, r"""Create the app. FastAPI automatically serves interactive docs at `/docs` (Swagger UI) and a machine-readable schema at `/openapi.json`."""),
        (20, 22, r"""`get_pipeline` builds the pipeline **once**: `@lru_cache` remembers the result, so every request shares one instance (a singleton). It is expensive (loads the embedding model and the index), so the first request pays a cold start. Endpoints obtain it via `Depends(get_pipeline)`, which is also how tests swap in a fake pipeline (`app.dependency_overrides`)."""),
        (25, 26, r"""`AskRequest` is the request body schema. `Field(min_length=1, max_length=2000)` makes pydantic reject empty or oversized questions automatically with HTTP 422, before our code runs."""),
        (29, 33, r"""`CitationOut`: the JSON shape of one citation."""),
        (36, 39, r"""`AskResponse`: the JSON shape of an answer. Declared as `response_model` below so responses are validated, filtered to these fields, and documented in `/docs`."""),
        (42, 44, r"""`GET /health` returns `{"status": "ok"}`. It deliberately does **not** depend on the pipeline, so it is instant and does not force the model to load. Container platforms poll this to decide whether the service is alive."""),
        (47, 49, r"""`GET /documents` lists what is indexed: source file name and chunk count."""),
        (52, 55, r"""`POST /documents` uploads a file (multipart form data); `status_code=201` means "Created". Note `def`, **not** `async def`: FastAPI runs plain functions in a worker thread pool, so slow blocking work (embedding, network calls) does not freeze the server's event loop for other users."""),
        (56, 56, r"""Sanitise the uploaded file name down to its last component."""),
        (57, 58, r"""Whitelist the extension. Anything else is HTTP **415 Unsupported Media Type**."""),
        (59, 59, r"""Read at most `MAX_UPLOAD_BYTES + 1` bytes. The extra byte is a trick: if we get more than the limit, the file is too big, and we know it without ever loading a huge file into memory."""),
        (60, 61, r"""Too big: HTTP **413 Payload Too Large**."""),
        (62, 65, r"""Try to parse the file. `ValueError` (unsupported type) and `PdfReadError` (corrupt PDF) become HTTP **422 Unprocessable Content**. `from exc` keeps the original error attached for debugging."""),
        (66, 67, r"""A file with no extractable text (for example a scanned PDF) is rejected with 422 rather than silently indexed as nothing."""),
        (68, 68, r"""Ingest it and return the counts (`{"documents": 1, "chunks": n}`)."""),
        (71, 72, r"""`POST /ask` takes an `AskRequest` (already validated), returns an `AskResponse`."""),
        (73, 74, r"""Call the pipeline. `asdict` recursively converts the `Answer` dataclass and its nested `Citation` objects into plain dictionaries for JSON."""),
        (75, 76, r"""Claude rate limit (HTTP 429 from Anthropic) becomes HTTP **429** for our client, who can retry later."""),
        (77, 78, r"""Bad or missing API key is *our* configuration problem, not the caller's: return **500** with a generic message (no details leaked)."""),
        (79, 80, r"""Network failure reaching Anthropic: HTTP **503 Service Unavailable**."""),
        (81, 82, r"""Any other error status from Anthropic: HTTP **502 Bad Gateway** (an upstream service failed). **Order matters:** `RateLimitError` and `AuthenticationError` are *subclasses* of `APIStatusError`, so the specific handlers must come before this general one, otherwise they would never run."""),
    ],
)
...
APP["app/cli.py"] = (
    ...
    [
        (1, 1, r"""Docstring."""),
        (2, 3, r"""`argparse` (standard library command-line parser) and `Path`."""),
        (5, 7, r"""Reuse the same settings, folder loader and pipeline builder as the API: **one pipeline, two front ends**."""),
        (10, 10, r"""`main` accepts an optional argument list. Passing `None` makes argparse read `sys.argv`; tests can pass their own list."""),
        (11, 12, r"""Create the parser (`prog` sets the name shown in help). `add_subparsers` creates the `ingest` / `ask` / `stats` sub-commands; `required=True` forces the user to pick one."""),
        (13, 14, r"""`ingest <directory>`: `type=Path` converts the text argument into a `Path`."""),
        (15, 16, r"""`ask "<question>"`."""),
        (17, 17, r"""`stats` takes no arguments."""),
        (18, 18, r"""Parse the command line into `args`. Bad input prints usage and exits."""),
        (20, 20, r"""Build the real pipeline (loads the embedding model and any saved index)."""),
        (21, 22, r"""`ingest`: read every supported file in the folder, index it, print the counts."""),
        (23, 24, r"""`stats`: print the source files and their chunk counts."""),
        (25, 29, r"""Otherwise it is `ask`: print the answer, then one line per citation with file, chunk number and similarity score."""),
        (32, 33, r"""Standard guard so `python -m app.cli ...` runs `main()`, but importing the module does not."""),
    ],
)
...
The single most useful habit in RAG work: measure retrieval separately from generation.""",
    {
        "@header": r"""Imports and `ROOT` (the project folder, found relative to this file so the script works from any directory).""",
...
BY_NAME["tests/test_llm.py"] = (
    r"""Tests for the generation layer, using a stub client so no real API call is made.""",
    {
        ...
        "StubClient": r"""Imitates `anthropic.Anthropic`: it exposes `messages.create(...)`, records the arguments it was called with, and returns a canned response.""",
