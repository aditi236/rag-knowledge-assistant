...
| No credentials configured (Claude selected, no key) | `llm.py` then `api.py` / `cli.py` | Clear `MissingCredentialsError`: HTTP 500 saying to set `ANTHROPIC_API_KEY`; CLI prints a one-line error |
